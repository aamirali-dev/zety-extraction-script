...
categories_div = soup.find("div", class_="blog-article")
...
for li in categories_div.find_all(recursive=False):
    ...
    category_div = li.find("div").find_all(recursive=False)[1]

    # Find subcategories and extract the text and href
    subcategories = {}
    for sub_li in category_div.find_all("a"):

        subcategory_text = sub_li.find('p').text
